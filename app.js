(() => {
  ...
  function paintSegment(name,p){ const s=segments[name]; if(!s)return; /* Keep guide tracks static. Repainting CSS gradients every animation frame caused compositor ghost trails on some Android GPUs. The moving orb already communicates progress. */ s.style.removeProperty('background'); }
